...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (modal && openBtn && closeBtn) {
    openBtn.addEventListener("click", () => {
      ...
      if (streamlitFrame && !streamlitFrame.src) {
        // Lazy-load the Streamlit app when modal opens
        const src = streamlitFrame.getAttribute('data-src');
        if (src) {
          streamlitFrame.src = src;
        }
      }
    });
    ...
    closeBtn.addEventListener("click", () => {
      ...
      // Optionally unload to free resources (keeps memory low on repeated opens)
      if (streamlitFrame) {
        streamlitFrame.src = '';
      }
    });
